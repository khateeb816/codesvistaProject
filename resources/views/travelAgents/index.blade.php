...
<div class="right_col" role="main">
    <div class="col-md-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                <form class="form-horizontal form-label-left" method="POST" action="{{ route('travelAgents.store') }}"
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            ...
                            <select id="Airline" name="arlines_deals_with" class="form-control select2 select2-offscreen" data-trackchange="true" data-required="required" data-validate="true" data-message="Airlines is required" tabindex="-1" title="">
                                <option> Select Airlines </option>
                                    ...
                                    <option value="{{ $airline->name }}" data-custom="{{ $airline->id }}">{{ $airline->name }}</option>
                                    @endforeach
                            </select>
                        </div>
                    ...
                    <div class="item form-group">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            ...
                            <input id="file_path" class="form-control" value="{{ old('address') }}"
                            ...
                            <table class="table table-striped table-bordered" id="table">
                                ...
                                <tbody>
                                    @foreach($travelAgents as $travelAgent)
                                        <tr>
                                            <td>{{ $travelAgent->name }}</td>
                                            <td>{{ $travelAgent->location }}</td>
                                            <td>{{ $travelAgent->primary_phone }}</td>
                                            <td>{{ $travelAgent->secondary_phone }}</td>
                                            <td>{{ $travelAgent->arlines_deals_with }}</td>
                                            <td>
                                                <a href="{{ route('travelAgents.edit', $travelAgent->id) }}" class="btn btn-info btn-sm">
                                                    <i class="fa fa-edit"></i> Edit
                                                </a>
                                                <form action="{{ route('travelAgents.destroy', $travelAgent->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
